...
class DwayApplication {
    ...
    fun dataLoader(batchService: BatchService, countryRepository: CountryRepository) = CommandLineRunner {
        batchService.countryBatchInsert(getListOfCountries())
        batchService.airportBatchInsert(getListOfAirports(countryRepository))
        batchService.aircraftBatchInsert(getListOfAircraft())
    }

    private fun getListOfCountries(): List<Country> {
        return listOf(
    ...
    private fun getListOfAirports(countryRepository: CountryRepository): List<Airport> {
    ...
    private fun getListOfAircraft(): List<Aircraft> {
        return listOf(
            Aircraft(model = "B737-800", manufacture = Manufacture.BOEING)
        )
    }
}
